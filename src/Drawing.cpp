...
Drawing::Drawing(Shapes shape, SDL_Texture *texture) noexcept
    : ClassName{"Drawing"}, Shape{shape}, TexturePtr{texture}
{
  switch (shape) {
...
void Drawing::loadTextures(SDL_Renderer *renderer, std::string directory) noexcept
{
  ...
  for (const auto &dirEntry :
    ...
    if (dirEntry.is_regular_file() &&
        dirEntry.path().filename().string().ends_with(".png")) [[likely]] {
...
const double Drawing::angle() const noexcept
{
  return Angle;
}

const float Drawing::interval() const noexcept
{
  return Interval;
}
...
void Drawing::blit(SDL_Renderer *renderer, const SDL_FRect *srcRect) noexcept
{
  switch (Shape) {
  case Shapes::Rectangle:
    blit<Shapes::Rectangle>(renderer, srcRect);
    break;
  case Shapes::Triangle:
    blit<Shapes::Triangle>(renderer, srcRect);
    break;
  }
}
