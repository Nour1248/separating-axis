...
class Convex : public gd::Collider {
public:
  Convex(Shapes s, SDL_Texture *t = nullptr) noexcept : gd::Collider{s, t}
  {
  }
...
int main()
{
  LOG_IF(SDL_Init(SDL_INIT_VIDEO) != 0, "COULDNT INIT SDL: ", SDL_GetError());
  LOG_IF(IMG_Init(IMG_INIT_PNG) != IMG_INIT_PNG, "COULDNT INIT IMG: {}/n",
         SDL_GetError());

  SDL_CreateWindowAndRenderer(600, 600, SDL_WINDOW_RESIZABLE, &window, &renderer);
  ...
  LOG_IF(!r1.texture() || !r2.texture(), "Couldnt load textures: {}/n",
         IMG_GetError());
  r1Ptr = &r1;
  r2Ptr = &r2;
